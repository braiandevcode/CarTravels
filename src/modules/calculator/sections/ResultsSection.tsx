...
export function ResultsSection({ onViewReceipt }: ResultsSectionProps) {
  ...
  if (state.total === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 gap-3 opacity-60">
        <div className="text-4xl opacity-30" role="img" aria-label="Calculadora">📊</div>
        <p className="text-center text-text-muted">Primero ingresá el total del día</p>
      </div>
    )
  }
  ...
  if (!state.calculated) {
    ...
    const hasValidVales = state.showVales && state.vales.length > 0
      ? state.vales.every((v) => v.trips > 0 && v.pricePerTrip > 0)
      : true
    const hasInvalidVales = state.showVales && state.vales.length > 0 && !hasValidVales
      ...
      <div className="flex flex-col gap-4 animate-fade-in">
        ...
        {hasInvalidVales && (
          <div className="p-2.5 rounded-xl bg-accent-red/10 border border-accent-red/20 flex items-start gap-2">
            <AlertTriangle className="h-4 w-4 text-accent-red flex-shrink-0 mt-0.5" aria-hidden="true" />
    ...
    <div className="flex flex-col gap-2 text-sm md:text-base animate-fade-in">
      ...
      <div className="rounded-xl bg-accent-amber/8 border border-accent-amber/15 p-3 space-y-2">
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-accent-amber font-display">+${result.agencyAmount.toLocaleString()}</span>
        </div>
