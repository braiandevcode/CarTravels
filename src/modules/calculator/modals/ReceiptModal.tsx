  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-md rounded-2xl bg-bg-card border border-border-subtle shadow-xl max-h-[90vh] flex flex-col animate-scale-in print-white"
      >
        <div className="flex items-center justify-between border-b border-border-subtle p-4 no-print">
          <h2 id={MODAL_TITLE_ID} className="text-lg font-bold text-text-primary font-display tracking-wide">Recibo de Jornada</h2>
          <button
            type="button"
            onClick={onClose}
            autoFocus
          ...
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 mb-2">
              <div className="w-10 h-10 rounded-full bg-accent-amber/20 flex items-center justify-center">
                <span className="text-xl" role="img" aria-label="Auto">🚗</span>
              </div>
            </div>
            <h1 className="text-2xl font-black text-text-primary font-display tracking-tight">carTravels</h1>
            <p className="text-xs text-text-muted mt-1 font-display">Resumen de jornada</p>
        ...
        {!result.isPercentValid && (
          <div className="mx-4 mb-2 p-3 rounded-xl bg-accent-red/10 border border-accent-red/30 flex items-start gap-2.5 no-print">
        ...
        <div className="flex flex-col gap-2 border-t border-border-subtle p-4 no-print">
          <Button variant="primary" onClick={handleDownload} disabled={!result.isPercentValid}>
            <Download className="h-4 w-4" aria-hidden="true" />
            {result.isPercentValid ? 'Descargar PDF' : 'Ajusta los porcentajes'}
          </Button>
          <Button variant="secondary" onClick={handleShare} disabled={!result.isPercentValid}>
            <Share2 className="h-4 w-4" aria-hidden="true" />
            {result.isPercentValid ? 'Compartir por WhatsApp' : 'Porcentajes incompletos'}
          </Button>
        </div>
      </div>
    </div>
